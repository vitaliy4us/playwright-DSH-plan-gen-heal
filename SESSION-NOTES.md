...
- Итог: **18 passed + 2 skipped из 20** (13 прежних + 7 новых; skipped — два теста, документирующие дефекты).

**Найденные циклом дефекты приложения:**
1. **Корзина теряется при перезагрузке сразу после добавления.**
   Отчёт: `Bug Reports/BUG-1-cart-is-lost-on-immediate-reload.md`.
   Приложение сбрасывает корзину в localStorage примерно через **1 с** после изменения (замерено: ~1009 мс).
   `page.reload()` сразу после «ADD TO CART» обгоняет запись, и старт приложения перезаписывает все ключи
   `app_data_*` строкой `"null"`. Тест `should-persist-cart-after-app-reload` помечен `test.fixme()`
   (не подгоняли под баг).
2. **Пустую корзину можно оформить (нет валидации).**
   Отчёт: `Bug Reports/BUG-2-empty-cart-can-be-ordered.md`.
   Ожидание вынесено в отдельный тест `should-not-offer-order-placement-when-the-cart-is-empty` → `test.fixme()`;
   проверено, что он падает **по правильной причине** (локатор найден, кнопка «Place Order» — `enabled`).
   Наблюдаемое поведение остаётся задокументировано зелёными тестами 1.5 и 1.6 — в плане они помечены
   как «observed behaviour», чтобы не выглядеть требованием.

**Разобрано в цикле, но дефектами не является:**
- экран подтверждения заказа держится дольше 5 с — ассерт URL требует `{ timeout: 15000 }`
  (та же практика, что в `tests/checkout/should-place-order-successfully.spec.ts`);
- `specs/cart-edge-cases.plan.md` усилен: добавлен раздел «Expected behaviour vs observed behaviour
  (known deviations)» со ссылками на отчёты и сценарий-требование 1.7.
...
## Капстоун: цикл на новом участке «Search and Cart» (2026-09-23)
Проверка навыка после курса — цикл пройден агентами на участке, которого не было ни в одном из планов.

- `specs/search-and-cart.plan.md` — новая группа Search and Cart, **3 сценария** (планировщик).
- `tests/search-and-cart/` — **2 теста сгенерированы и прошли с первого прогона** (хилер не понадобился):
  добавление товара из отфильтрованного поиска и сохранение корзины при очистке поиска.
- Третий сценарий (`should-search-case-insensitively`) тоже закрыт тестом — план и тесты теперь 1:1.

**Новые факты о приложении (проверено вживую):**
- поиск фильтрует по подстроке **без учёта регистра**: «ca» и «CA» дают один и тот же набор —
  Cauliflower, Carrot, Capsicum, Cashews (ровно 4, порядок одинаковый);
- приложение **УДАЛЯЕТ** несовпавшие карточки из DOM (в `.products` только совпадения: 4 при «ca», 30 при пустом запросе);
- **ловушка локатора**: голый `.product` дополнительно матчит посторонний скрытый узел с классами
  `showPriceWrapper product` вне `.products` → использовать `.products > .product`;
- корзина **не зависит** от фильтра: очистка поиска возвращает все 30 товаров, корзина сохраняется;
- **ловушка**: переход на тот же URL с тем же хешем **не перезагружает SPA** (значение поиска осталось) —
  для чистого состояния нужен настоящий `page.reload()`;
- очистка поля работает через обычный `input`-эвент (`fill('')`), а не только через `keyup`.

**Изменение конфигурации DSH:** браузерному MCP добавлена capability `testing`
(`args: ['-y', '@playwright/mcp@latest', '--caps=testing']`). Проверено: `browser_verify_text_visible` и
`browser_generate_locator` появились hot-swap, без перезапуска. Это разгадка трёх «не найденных инструментов»:
`browser_check` и `browser_reload` отсутствуют в core MCP, а `browser_resume` требует capability `devtools`.

**Итог:** 23 теста — **21 passed + 2 skipped**, регрессий нет.

**Упаковка проекта (шаг «показать результат»):**
- `README.md` — входная точка репозитория: что лежит в каждой папке, таблица покрытия по группам,
  как получены тесты (planner → generator → healer), как запустить, найденные дефекты, ограничения.
- `.github/workflows/playwright.yml` — CI: `npm ci` → установка Chromium → `npm test` → HTML-отчёт артефактом;
  триггеры: push в master, pull request, ручной запуск.
- `package.json` — добавлены отсутствовавшие скрипты `test`, `test:headed`, `test:ui`, `report`, `trace`
  и описание пакета. Согласованность `package-lock.json` проверена (для `npm ci` в CI).
- **Важно про CI:** suite ходит на живой внешний сайт, поэтому красный прогон может означать недоступность
  третьей стороны, а не регрессию. В `playwright.config.ts` уже есть `retries: 2` и `workers: 1` для CI.
...
## Локализация документации (2026-09-23)

Сводный документ собран в трёх языках. Готовые файлы лежат **вне репозитория**, в `C:\Users\vital\AI`:

| Файл | Язык | Размер |
|---|---|---|
| `Playwright-Test-Agents-Documentation.docx` | русский | 149.1 КБ |
| `Playwright-Test-Agents-Documentation-EN.docx` | английский | 132.5 КБ |
| `Playwright-Test-Agents-Documentation-UK.docx` | украинский | 151.7 КБ |

Во всех трёх — **37 документов и 58 таблиц**, одинаковая структура заголовков, ноль непереведённого текста.

**Сборщик русской версии:** `C:\Users\vital\AI\build-docx.py` — конвертер markdown → docx на python-docx
(стили заголовков Word, таблицы с рамками, блоки кода на серой заливке, frontmatter отдельным блоком,
номера страниц в колонтитуле). Перезапуск: `python C:\Users\vital\AI\build-docx.py`.

**Конвейер локализации — в репозитории, папка `docx-i18n/`:**
- `en/`, `uk/` — переведённые markdown-исходники, разложены по частям (`course/`, `greenkart/`, `conduit/`);
- `build-i18n.py en|uk` — сборка локализованного `.docx`; переиспользует конвертер из `build-docx.py`,
  поэтому оформление идентично русской версии; не падает на неполном наборе — печатает список отсутствующих;
- `check-translations.py` — проверка качества: кириллица в EN-дереве (норма 0) и буквы `ы/ъ/э/ё` в UK-дереве
  (норма 0 — их нет в украинском алфавите);
- `README.md` — правила перевода, глоссарий, порядок пересборки.

**Как делалось:** 9 фоновых субагентов (по частям, каждый в своём контексте), сборка — моя, из переведённой
разметки. Документы, уже написанные по-английски (README и три плана GreenKart, определения агентов,
README и план Conduit, промпты), в EN-версии не переводились — скопированы как есть.

**Ошибка в задании, найденная проверкой.** Первая версия правила запрещала агентам трогать содержимое
блоков кода — из-за чего в переводах остались русские комментарии, ASCII-схема агентной петли и три
шаблона `SKILL.md`. Правило исправлено (код не трогать, а документацию внутри блока переводить), правки
внесены во все затронутые файлы, документация конвейера обновлена.

**Наблюдение о субагентах.** Отчёты фоновых агентов приходят с задержкой и могут описывать устаревшее
состояние: три отчёта противоречили фактам, один предупреждал о непереведённых файлах, которые к тому
моменту были исправлены. Надёжный источник истины — содержимое файлов на диске; проверять результат
скриптом, а не по отчёту.

**Открытые вопросы (на решение):**
1. Судьба `docx-i18n/` в репозитории: коммитить только инструменты + `.gitignore` для переводов,
   коммитить всё целиком или удалить переводы, оставив скрипты.
2. Точечные правки перевода: качество машинное, при чтении возможны неудачные формулировки —
   править можно без агентов (они больше не пишут в файлы) и пересобирать одной командой.

**Что дальше (обновлённый список):**
1. Решить судьбу `docx-i18n/` (см. выше).
2. Бэклог Conduit из `playwright-DSH-mcp-cli/specs/conduit.plan.md`: валидация входа, редактирование
   статьи, комментарии — каждый сценарий требует живого исследования перед генерацией.
3. CI в проекте Conduit — сознательно не добавлен: набор мутирует общие публичные данные
   (создаёт и удаляет статьи на живом демо-сайте).
4. Практика: капстоун из урока 6, навыки `playwright-component-testing` и `playwright-trace`.
